...
前往 [Releases](https://gitee.com/lizhi-studio/xcopy/releases) 下载最新版 zip，解压后将 `ClipVault.app` 拖入 `/Applications` 即可。

### 从源码构建

```bash
git clone https://gitee.com/lizhi-studio/xcopy.git
cd xcopy
./scripts/bundle.sh
...
3. 按 `⌃⌘V`（Control+Command+V）打开历史面板
4. 点击任意记录即可粘贴到当前应用
...
使用 `release.sh` 一键打包并上传到 Gitee Releases：

```bash
./release.sh                        # 打包（使用当前版本号）
./release.sh -v 1.2                 # 指定版本号打包
./release.sh --upload               # 打包 + 上传 Gitee
./release.sh -v 1.2 --upload        # 指定版本号 + 上传
```

上传功能需要在项目根目录创建 `.release.env`：
